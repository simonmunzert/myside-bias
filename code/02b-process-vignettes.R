...
vignettes_content_df <- bind_rows(list(vignettes_content_bra_df, vignettes_content_col_df, vignettes_content_gbr_df, vignettes_content_ger_df, vignettes_content_idn_df, vignettes_content_ind_df, vignettes_content_indeng_df, vignettes_content_nig_df, vignettes_content_phl_df, vignettes_content_phleng_df, vignettes_content_pol_df, vignettes_content_tur_df, vignettes_content_usa_df))
saveRDS(vignettes_content_df, file = "data/cooked/vignettes_content_df.rds")
...
vignettes_content_df <- readRDS("data/cooked/vignettes_content_df.rds")
vignettes_content_df_sub <- dplyr::select(vignettes_content_df, -contains("split"), -contains("lines"))
write_csv(vignettes_content_df_sub, "data/cooked/vignettes_content_df.csv")
...
data_survey_vig_long$vig_perc_hate2[data_survey_vig_long$vig_perc_offensive == 1]
...
data_survey_combined$target_resp_issue_agreement <- factor(data_survey_combined$target_resp_issue_agreement, levels = c("Disagree", "Agree"))


# generate issue position / ideology match indicator (for an ideology specification of Hypothesis 1)

data_survey_combined <- 
  data_survey_combined %>% mutate(
    target_resp_ideology_match = 
      case_when(target_position == "left" & leftright_cat == "Left" ~ "Match",
                target_position == "left" & leftright_cat == "Right" ~ "Mismatch",
                target_position == "right" & leftright_cat == "Right" ~ "Match",
                target_position == "right" & leftright_cat == "Left" ~ "Mismatch",
                .default = NA)
  )
data_survey_combined$target_resp_ideology_match <- factor(data_survey_combined$target_resp_ideology_match, levels = c("Mismatch", "Match"))
...
data_survey_combined$target_ethnicity_match <- factor(data_survey_combined$target_ethnicity_match, levels = c("Mismatch", "Match"))
data_survey_combined$sender_ethnicity_match <- factor(data_survey_combined$sender_ethnicity_match, levels = c("Mismatch", "Match"))
...
data_survey_combined$target_white_match <- factor(data_survey_combined$target_white_match, levels = c("Mismatch", "Match"))
data_survey_combined$sender_white_match <- factor(data_survey_combined$sender_white_match, levels = c("Mismatch", "Match"))
...
data_survey_combined$target_gender_match <- factor(data_survey_combined$target_gender_match, levels = c("Mismatch", "Match"))
data_survey_combined$sender_gender_match <- factor(data_survey_combined$sender_gender_match, levels = c("Mismatch", "Match"))
...
data_survey_combined$genderXcategory <- interaction(data_survey_combined$gender2, data_survey_combined$sender_category)

# ageXcategory

data_survey_combined$ageXcategory <- interaction(data_survey_combined$age_cat2, data_survey_combined$sender_category)
